 ...
 */
BTree.prototype.searchFirst = function(key, f) {
    return this.root.searchFirst(key, f);
};
 ...
 * search for a key
 *
 * @return {Cursor} a promise of the cursor pointing to the first key found
 */
BTreeNode.prototype.search = function(key) {
    var i = 0;
    while (i <= this.data.length - 1 && key > this.data[i].key) i++;
    if (i <= this.data.length - 1 && key === this.data[i].key) return Q(new Cursor(this, i));
    if (this.isLeaf()) return Q(null);
    return this.tree.provider.getChild(this.children[i])
        .then(function(child) { return child.search(key); });
};
 ...
 * @return {Cursor}         a promise of the curosr pointing to the found key, or null.
 */
BTreeNode.prototype.searchFirst = function (key, f) {
    ...
    function searchKey() {
        ...
        return i < that.data.length - 1 && key === f(that.data[i + 1].key) ?
            new Cursor(that, i + 1) : null;
    }

    return this.isLeaf() ?
         Q(searchKey()) :
         this.tree.provider.getChild(this.children[i + 1])
            .then(function (child) { return cihld.searchFirst(key, f); })
            .then(function (found) { return Q(found ? found : searchKey()); });
};
...
BTreeNode.prototype.insertNonFull = function(key, payload) {
    ...
    if (this.isLeaf()) {
        ...
        child.insertNonFull(key, payload);
    }
};
...
Cursor.prototype.moveNext = function() {
    if (this.node.isLeaf()) {
        ...
        var first = this.node.children[this.iKey + 1].getFirst();
