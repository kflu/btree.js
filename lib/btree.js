...
function BTree(minDegree, provider, cb) {
    ...
    provider.createNode(this, null, function (err, node) {
        if (err) { cb(err); } else {
            that.root = node;
            cb(undefined, that);
        }
...
BTree.prototype.search = function(key, cb) {
    return this.root.search(key, cb);
};
...
BTree.prototype.searchFirst = function(key, f, cb) {
    return this.root.searchFirst(key, f, cb);
};
...
BTreeNode.prototype.search = function(key, cb) {
    ...
    if (i <= this.data.length - 1 && key === this.data[i].key)
        cb(undefined, new Cursor(this.tree, this.tree.provider.getId(this), i));
    if (this.isLeaf()) cb(undefined, null);

    this.tree.provider.load(this.children[i], function (error, child) {
        if (error) {
            cb(error, child);
        } else {
            child.search(key, cb);
...
BTreeNode.prototype.searchFirst = function (key, f, cb) {
    ...
    while (i >= 0 && key <= f(this.data[i].key)) i--;
...
BTreeNode.prototype.insertNonFull = function(key, payload, cb) {
    ...
    if (this.isLeaf()) {
        ...
        this.tree.provider.save(this, function (err) {
            cb(err);
        });
    } else {
        i = 0;
        while (i <= this.data.length - 1 && key >= this.data[i].key) i++;
        this.tree.provider.load(this.children[i], function (err, child) {
            if (err) { cb(err); } else {
                if (child.isFull()) {
                    that.splitChild(i, function (err) {
                        if (err) { cb(err); } else {
                            if (key >= that.data[i].key) i++;
                            that.tree.provider.load(that.children[i], function (err, child) {
                                if (err) { cb(err); } else {
...
BTreeNode.prototype.splitChild = function(childIndex, cb) {
    ...
    that.tree.provider.load(that.children[childIndex], function (err, left) {
        if (err) { cb(err); }
            ...
            that.tree.provider.createNode(that.tree, provider.getId(that), function (err, right) {
                if (err) { cb(err); } else {
                    withLeftRight(left, right);

                    // saving changes to the current, left, right nodes
                    provider.save(that, function (err) {
                        if (err) { c(err); } else {
                            provider.save(left, function (err) {
                                if (err) { c(err); } else {
                                    provider.save(right, function (err) {
                                        cb(err);
                                    });
                                }
                            });
    ...
    function withLeftRight(left, right) {
        ...
        that.children[childIndex + 1] = that.tree.provider.getId(right);
...
BTreeNode.prototype.getFirst = function(cb) {
    if (this.isLeaf()) {
        cb(undefined, new Cursor(this.tree, this.tree.provider.getId(this), 0));
    }

    this.tree.provider.load(this.children[0], cb);
};
...
Cursor.prototype.getData = function(cb) {
    var that = this;
    this.tree.provider.load(this.node, function (err, child) {
        if (err) { cb(err); }
...
Cursor.prototype.moveNext = function(cb) {
    ...
    provider.load(this.node, function (err, node) {
        if (node.isLeaf()) {
